...
class Channel(Base):
    ...
    discounts = Column(ForeignKey("discount.id"), nullable=False)
    entrance_fees = Column(ForeignKey("entrance_fee.id"), nullable=False)
    fees = Column(ForeignKey("fee.id"), nullable=False)
    grants = Column(ForeignKey("grant.id"), nullable=False)
    pricing_rule = relationship("PricingRule", back_populates="channels")
    items = Column(ForeignKey("item.id"), nullable=False)
    number_of_guests = Column(Integer, nullable=False)
    payments = Column(ForeignKey("payment.id"), nullable=False)
    restaurant_id = Column(String(256), ForeignKey("restaurant.id"))
    ...
    tags = Column(ForeignKey("tag.id"), nullable=False)
    taxes = Column(ForeignKey("tax.id"), nullable=False)
    ...
    customer = Column(ForeignKey("customer.id"), nullable=False)
    delivery_address = Column(ForeignKey("delivery_address.id"), nullable=False)
